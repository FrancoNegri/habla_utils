...
Podemos observar en la figura \ref{generalNormalizado} los nuevos resultados con los datos normalizados. Para $40\%$ y $50\%$ inglés . Para $60\%$ . Para $70\%$ .
...
De estos resultados podemos observar que con $30\%$ de interpolación de inglés, los participantes coinciden en que la voz puede atribuirse a una persona de habla nativa española.
...
Observando las oraciones una por una, podemos observar algunas particularidades. para $40\%$ inglés, puede verse que no hay una votación homogénea. Por ejemplo en la figura \ref{tresNueve} en la oración $3$ el $80\%$ de los participantes coincide que la voz pertenece a un hablante de habla hispana, mientras que en la oración $9$ el $60.00\%$ de los participantes considera que la voz pertenece a un hablante de habla anglosajona.
...
En la figura \ref{resultadosGeneralesNacVsPlot} presentamos los resultados comparando las distancias de Levenshtein con los porcentajes de participantes que determinaron que el hablante fuera anglosajon. Al igual que en el aparatado \ref{SeccionInteligibilidad}, en el eje $x$ presentamos los grados de interpolación de ingles, yendo desde $30\%$ hasta $70\%$. El eje $y$ presentamos la distancia de Levenshtein  entre la oración objetivo y aquella transcripta por por cada participante del lado izquierdo, y el porcentaje de participantes que atribuyeron la voz a un hablante anglosajón del lado derecho. Cada uno de los boxplots describe la distancia mínima obtenida y la distancia máxima (los bigotes), como así también el primer y tercer cuartil (el piso y el techo de la caja) y la distancia media (linea interior que atraviesa la caja). Adicionalmente podemos observar con círculos vacíos los outliers de la muestra.
...
En esta figura podemos apreciar que para cada aumento en el grado de interpolación de ingles, tanto el porcentaje de participantes que considera la voz como un hablante anglosajón como la media de la distancia de Levenshtein crecen de manera lineal. Comenzando con un $5\%$ de participantes afirmando que la voz anglosajona y un una media de la distancia de Levenshtein de $3$ para una interpolación de $30\%$ ingles - $70\%$ castellano, y llegando hasta $55\%$ de participantes afirmando que la voz pertenecía a un hablante anglosajón y una distancia de Levenshtein media de aproximadamente $18$ caracteres para la interpolación de $70\%$ ingles - $30\%$ castellano.

En base a estos resultados podemos afirmar que la hipótesis original tiene cierto grado de validez experimental: la interpolación de HMMs descripta en esta tesis es un método valido para generar voces que pueden ser identificadas con un nativo anglosajón hablando castellano. Sin embargo esto viene con un costo asociado, la claridad de las oraciones disminuirá a medida que se aumenta el grado de interpolación del modelo de ingles.

Éxitos!!

Puedo terminar acá o reiterar sobre el lado oscuro de los resultados: separado por oración algunas contradicen la hipótesis general, por ejemplo.

\begin{figure}
\begin{center}$
\begin{array}{lll}
\includegraphics[width=.7\textwidth]{imagenes/nacVsPlot/6.png}
\end{array}$
\end{center}
\caption{Distancia de Levenshtein para distintos grados de interpolación}
\end{figure}